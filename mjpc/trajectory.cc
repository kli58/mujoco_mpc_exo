...
void Trajectory::CustomRollout(
    std::function<void(double* action, const double* state, double time, double* userdata)>
        policy,
    const Task* task, const mjModel* model, mjData* data, const double* state,
    double time, const double* mocap, const double* userdata, int steps) {
  CustomNoisyRollout(policy, task, model, data, state, time, mocap, userdata,
                     /*xfrc_std=*/0, /*xfrc_rate=*/1, steps);
}

void Trajectory::CustomNoisyRollout(
    std::function<void(double* action, const double* state, double time, double* userdata)>
        policy,
    const Task* task, const mjModel* model, mjData* data, const double* state,
  ...
  for (int t = 0; t < horizon - 1; t++) {
    ...
    //update user data based on state
    task->UpdateUserData(model,data);
